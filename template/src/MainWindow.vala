namespace Splash {
    public class MainWindow : Hdy.ApplicationWindow {
        ...
        public MainWindow (Splash.Application application) {
            Object (
                ...
                icon_name: "com.github.BharatKalluri.double_icon_flatpak_bug"
            );
        }
        ...
        private void restore_window_state () {
            ...
            Splash.saved_state.get ("window-size", "(ii)", out rect.width, out rect.height);

            default_width = rect.width;
            default_height = rect.height;

            var window_state = Splash.saved_state.get_enum ("window-state");
            switch (window_state) {
                ...
                case WindowState.FULLSCREEN:
                    ...
                    break;
                default:
                    Splash.saved_state.get ("window-position", "(ii)", out rect.x, out rect.y);
        ...
        private void preserve_window_state () {
            ...
            if (Gdk.WindowState.MAXIMIZED in state) {
                Splash.saved_state.set_enum ("window-state", WindowState.MAXIMIZED);
            } else if (Gdk.WindowState.FULLSCREEN in state) {
                Splash.saved_state.set_enum ("window-state", WindowState.FULLSCREEN);
            } else {
                Splash.saved_state.set_enum ("window-state", WindowState.NORMAL);
                // Save window size
                int width, height;
                get_size (out width, out height);
                Splash.saved_state.set ("window-size", "(ii)", width, height);
            }

            int x, y;
            get_position (out x, out y);
            Splash.saved_state.set ("window-position", "(ii)", x, y);
        }
